...
public class PasswordHelp {
    ...
    public static void savePassword(Context context, String number, String password, boolean isRemember) {
        //对数据进行加密
        //得到key
        SecretKey key = PasswordHelp.readKey(PasswordHelp.getPath(context,"a.a"));
        if (key == null) {
            key = get3DESKey();
            //保存key
            PasswordHelp.saveKey(key, PasswordHelp.getPath(context,"a.a"));
        }
    ...
    public static String[] readPassword(Context context) {
        ...
        str[0] = d(context, str[0]);
        str[1] = d(context, str[1]);
        return str;
    }
    ...
    private static String d(Context context, String str) {
        if (!TextUtils.isEmpty(str)) {
            //对数据进行解密
            SecretKey key = readKey(PasswordHelp.getPath(context,"a.a"));
            if (key != null) {
